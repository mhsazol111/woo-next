...
const ProductGridItem = ({ product }) => {
  return (
    // <div key={product.id} className="product-item 2xl:w-1/6 xl:w-1/5 md:w-2/6 w-full ml-2 mr-2">
    <Link href={`/products/${product.slug}`}>
      <a key={product.id} className={`${productStyle.product_item} second_level_blur rounded-md p-5 relative overflow-hidden`}>
        {console.log(product)}
